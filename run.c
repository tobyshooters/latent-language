...
char *decode(Tokenizer *t, int prev_token, int token) {
  ...
  if (sscanf(piece, "<0x%02hhX>", &byte_val) == 1) {
    // ok this token is a raw byte token, careful to only print printable chars
    // or whitespace some of the other bytes can be various control codes,
    // backspace, etc. => skip
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler,
              char *prompt, int steps, int target_token) {
...
int main(int argc, char *argv[]) {
  ...
  // run!
  generate(&transformer, &tokenizer, &sampler, prompt, steps, target_token);
